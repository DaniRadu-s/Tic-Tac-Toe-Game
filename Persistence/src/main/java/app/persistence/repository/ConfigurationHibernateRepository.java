...
@Repository
public class ConfigurationHibernateRepository implements IConfigurationRepository {

    private static final Logger logger = LogManager.getLogger();

    public ConfigurationHibernateRepository() {
        logger.info("Initializing ConfigurationHibernateRepository");
    }
    ...
    public Iterable<Configuration> findAll() {
        final AtomicReference<Iterable<Configuration>> result = new AtomicReference<>();

        HibernateUtils.getSessionFactory().inTransaction(session -> {
            result.set(session.createQuery("from Configuration", Configuration.class).getResultList());
        });
