...
public class ClientJsonWorker implements Runnable, IObserver {
    ...
    private Response handleRequest(Request request) {
        ...
        if (request.getType() == RequestType.LOGIN) {
            ...
            try {
                Player logged = server.login(player, this);
                Response responsePlayer = JsonProtocolUtils.createOkResponse();
                responsePlayer.setPlayer(logged);
        ...
        if (request.getType() == RequestType.ADD_POSITION) {
            ...
            Position game = request.getPosition();
            try {
                server.addPosition(game);
                return okResponse;
            } catch (AppException e) {
                return JsonProtocolUtils.createErrorResponse(e.getMessage());
            }
        ...
        if (request.getType() == RequestType.ADD_CONFIGURATION) {
            ...
            try {
                ...
                Response responseCfg = okResponse;
                responseCfg.setConfiguration(configuration);
                return responseCfg;
            } catch (AppException e) {
                return JsonProtocolUtils.createErrorResponse(e.getMessage());
            }
        ...
        if (request.getType() == RequestType.GET_POSITIONS) {
            System.out.println("Get all positions request..." + request.getType());
            try {
                return JsonProtocolUtils.createGetAllPositionsResponse(server.getAllPositions());
            } catch (AppException e) {
                return JsonProtocolUtils.createErrorResponse(e.getMessage());
            }
        }
